...
The `docker` job is a 4-way matrix (read-values, write-values, subscriber, frontendsvc) running the canonical pre-push hardening pipeline: build for scanning → Trivy CRITICAL/HIGH gate → smoke test → `linux/amd64` push (`provenance: false` + `sbom: false` to keep the GHCR Packages UI "OS / Arch" tab functional) → cosign keyless OIDC signing by digest. **arm64 was dropped for build speed** (QEMU emulation was the dominant wall-time cost); re-add `linux/arm64` to the `docker` job's `platforms:` + restore `setup-qemu-action` to publish multi-arch again.

Each service has a multi-stage `Dockerfile` (golang:1.26-alpine builder → alpine:3.23 runtime). The runtime stage runs as non-root UID 10001 with `/app/main` owned by `app:app`. The `BUILDPLATFORM` directive on the builder stage cross-compiles the Go binary to the requested target platform via `GOOS`/`GOARCH` build args — so the Dockerfile is arch-agnostic and needs no change to add/drop an arch; the pipeline currently requests only `linux/amd64` (built natively on the amd64 runner, no QEMU).
