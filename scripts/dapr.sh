...
if [[ $SCRIPT_ACTION == "deploy" ]]; then
  ...
  helm repo update && \
  helm upgrade --install dapr dapr/dapr --set version=1.15.10 --namespace dapr-system --create-namespace --wait && \
  helm upgrade --install dapr-dashboard dapr/dapr-dashboard --set version=1.15.0 --namespace dapr-system --set serviceType=LoadBalancer --wait && \
